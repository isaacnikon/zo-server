...
IMAGE_BASE = 0x400000
PATCH_ADDRESS = 0x4495CF
PATCH_OFFSET = PATCH_ADDRESS - IMAGE_BASE
ORIGINAL_BYTES = bytes.fromhex("0f8501040000")
PATCHED_BYTES = b"\x90" * len(ORIGINAL_BYTES)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Patch gc12.exe so ordinary networked self item use applies "
            "local HP/MP changes before sending the request."
        )
    )
    parser.add_argument(
        "binary",
