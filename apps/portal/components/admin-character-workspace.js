...
function describeTradeState(value) {
  ...
  if (Number.isInteger(normalized) && normalized > 0) {
    const dateLabel = formatDate(new Date(normalized * 1000).toISOString());
...
function formatTradeStateBadge(value) {
  ...
  if (Number.isInteger(normalized) && normalized > 0) {
    const nowSeconds = Math.floor(Date.now() / 1000);
    return normalized <= nowSeconds ? 'Timed lock expired' : `Timed until ${formatDate(new Date(normalized * 1000).toISOString())}`;
  }
...
export default function AdminCharacterWorkspace({ profile, mapCatalog = [] }) {
      ...
      <section className="panel workspace-summary-panel">
        ...
        <div className="workspace-stat-band">
          ...
          <article className="workspace-stat-tile">
            ...
            <p>
              {profile.x}, {profile.y} · map #{profile.map_id} · updated {formatDate(profile.updated_at)}
            </p>
          </article>
        </div>
      ...
      <div className="workspace-columns">
        <section className="panel workspace-section-panel">
          <div className="section-heading">
                  ...
                  <span className="rail-count">{inventoryItems.length} entries</span>
                </div>

          {inventoryItems.length < 1 ? (
            <article className="muted-card">No items stored for this character.</article>
          ) : (
            <div className="inventory-editor-grid">
              {inventoryItems.map((item) => (
                <article className="inventory-editor-card" key={`${item.inventory_scope}-${item.instance_id}`}>
                  ...
                  {item.item_description ? <p className="inventory-copy">{item.item_description}</p> : null}
          ...
          <section className="panel workspace-section-panel">
            ...
            <div className="snapshot-grid">
              <article className="snapshot-card">
                ...
                <p>Across bag and warehouse tables.</p>
              </article>
              <article className="snapshot-card">
