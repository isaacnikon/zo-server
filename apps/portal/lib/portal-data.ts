import { createGamePasswordDigest, createPasswordHash } from './auth';
...
export async function listQuestDefinitions() {
  const result = await query(
    `SELECT
      ...
      q.repeatable,
      q.next_quest_id,
      COALESCE(next_quest.name, CONCAT('Quest #', q.next_quest_id::text)) AS next_quest_name,
      COALESCE(q.raw_data->>'type', '') AS quest_type,
      COALESCE(q.raw_data->>'acceptMessage', '') AS accept_message,
      COALESCE(q.raw_data->>'completionMessage', '') AS completion_message,
      CASE
        WHEN jsonb_typeof(q.raw_data->'steps') = 'array'
        THEN jsonb_array_length(q.raw_data->'steps')
        ELSE 0
      END AS step_count,
      CASE
        WHEN jsonb_typeof(q.raw_data->'prerequisiteTaskIds') = 'array'
        THEN jsonb_array_length(q.raw_data->'prerequisiteTaskIds')
        ELSE 0
      END AS prerequisite_count,
      COALESCE(q.raw_data->'steps'->0->>'description', '') AS first_step_description,
      COALESCE(active_stats.active_character_count, 0) AS active_character_count,
      COALESCE(completed_stats.completed_character_count, 0) AS completed_character_count,
      q.raw_data
    FROM game_quest_definitions q
    LEFT JOIN game_quest_definitions next_quest
      ON next_quest.quest_id = q.next_quest_id
    LEFT JOIN (
      SELECT
        quest_id,
      ...
      ON active_stats.quest_id = q.quest_id
    LEFT JOIN (
      SELECT
        quest_id,
        COUNT(*)::int AS completed_character_count
      FROM character_completed_quests
      GROUP BY quest_id
    ) completed_stats
      ON completed_stats.quest_id = q.quest_id
    ORDER BY q.min_level ASC, q.quest_id ASC`
  );
