...
function parseLoginPacket(payload: Buffer): { username: string; accountKey: string; passwordDigest: string } | null {
  if (payload.length < 6) {
    return null;
  }
  ...
  let passwordDigest = '';
  const passwordLenOffset = usernameEnd;
  if (passwordLenOffset + 2 <= payload.length) {
    const passwordLen = payload.readUInt16LE(passwordLenOffset);
    const passwordStart = passwordLenOffset + 2;
    const passwordEnd = passwordStart + passwordLen;
    if (passwordLen > 0 && passwordEnd <= payload.length) {
      passwordDigest = payload.slice(passwordStart, passwordEnd).toString('latin1').replace(/\0.*$/, '');
    }
  }

  const accountKey = username;
  return { username, accountKey, passwordDigest };
}
