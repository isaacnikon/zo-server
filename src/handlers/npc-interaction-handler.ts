...
function handleNpcInteractionRequest(session: GameSession, request: ServerRunRequestData): boolean {
  ...
  if (handleDarknessGuardReturnRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} darknessGuardReturn=1`
    );
    return true;
  }
  ...
  if (handleReceiverSpiritReturnRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} receiverSpiritReturn=1`
    );
    return true;
  }
  ...
  if (handleMirrorLakeSpiritReturnRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} mirrorLakeSpiritReturn=1`
    );
    return true;
  }
  ...
  if (handleMirrorLakeSpiritPalaceRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} mirrorLakeSpiritPalace=1`
    );
    return true;
  }
  ...
  if (handleBlurredLakeSpiritPalaceRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} blurredLakeSpiritPalace=1`
    );
    return true;
  }
  ...
  if (handleReceiverGhostMirrorLakeRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} receiverGhostMirrorLake=1`
    );
    return true;
  }
  ...
  if (handleLongicornSoldierSomberAisleRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} longicornSoldierSomberAisle=1`
    );
    return true;
  }
  ...
  if (handleBeetleGuideSilentHillRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} beetleGuideSilentHill=1`
    );
    return true;
  }
  ...
  if (handleFrontierSoldierSpringForestRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} frontierSoldierReturn=1`
    );
    return true;
  }
  ...
  if (handleLionCaptainPassRequest(session, resolvedNpcId, request)) {
    session.log(
      `NPC interaction sub=0x${request.subcmd.toString(16)} resolvedNpcId=${resolvedNpcId} requestedNpcId=${requestNpcId} rawNpcKey=${Number.isInteger(request.rawArgs?.[0]) ? request.rawArgs[0] : 0} scriptId=${Number.isInteger(request.scriptId) ? request.scriptId : 0} map=${session.currentMapId} lionCaptainPass=1`
    );
    return true;
  }
...
function tryStartCranePassGuardianNpcCombat(
  session: GameSession,
  npcId: number,
  ...
  session.sendCombatEncounterProbe({
    ...
    guardianApproachSide: isPointInsideBounds(
      session.currentX >>> 0,
      session.currentY >>> 0,
      CRANE_PASS_ZA2_APPROACH_POLYGON
    )
      ? 'za2'
      : 'crane-pass',
    encounterProfile: {
...
function tryStartSwanPassGuardianNpcCombat(
  session: GameSession,
  npcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.sendCombatEncounterProbe({
    ...
    originX: session.currentX >>> 0,
    originY: session.currentY >>> 0,
    guardianApproachSide: isPointWithinRadius(
      session.currentX >>> 0,
      session.currentY >>> 0,
      SWAN_PASS_ZA2_APPROACH_X,
      SWAN_PASS_ZA2_APPROACH_Y,
      SWAN_PASS_ZA2_APPROACH_RADIUS
    )
      ? 'za2'
      : 'swan-pass',
    encounterProfile: {
...
function tryStartLionCaptainNpcCombat(
  session: GameSession,
  npcId: number,
  request: ServerRunRequestData
): boolean {
...
function handleOrchidTempleReturnRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Orchid Temple return scene-enter map=${CLOUD_CITY_MAP_ID} pos=${CLOUD_CITY_RETURN_X},${CLOUD_CITY_RETURN_Y}`
  );
  session.sendSceneEnter(CLOUD_CITY_MAP_ID, CLOUD_CITY_RETURN_X, CLOUD_CITY_RETURN_Y);
  return true;
}

function handleDarknessGuardReturnRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Darkness Guard return scene-enter map=${GOLDEN_PATH_MAP_ID} pos=${GOLDEN_PATH_RETURN_X},${GOLDEN_PATH_RETURN_Y}`
  );
  session.sendSceneEnter(GOLDEN_PATH_MAP_ID, GOLDEN_PATH_RETURN_X, GOLDEN_PATH_RETURN_Y);
  return true;
}

function handleReceiverSpiritReturnRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
...
function handleMirrorLakeSpiritReturnRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Mirror Lake spirit scene-enter map=${BLURRED_LAKE_MAP_ID} pos=${BLURRED_LAKE_ENTRY_X},${BLURRED_LAKE_ENTRY_Y}`
  );
  session.sendSceneEnter(BLURRED_LAKE_MAP_ID, BLURRED_LAKE_ENTRY_X, BLURRED_LAKE_ENTRY_Y);
  return true;
}

function handleMirrorLakeSpiritPalaceRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Mirror Lake spirit palace scene-enter map=${MIRROR_PALACE_MAP_ID} pos=${MIRROR_PALACE_ENTRY_X},${MIRROR_PALACE_ENTRY_Y}`
  );
  session.sendSceneEnter(MIRROR_PALACE_MAP_ID, MIRROR_PALACE_ENTRY_X, MIRROR_PALACE_ENTRY_Y);
  return true;
}

function handleBlurredLakeSpiritPalaceRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Blurred Lake spirit scene-enter map=${DEEP_CORRIDOR_MAP_ID} pos=${DEEP_CORRIDOR_ENTRY_X},${DEEP_CORRIDOR_ENTRY_Y}`
  );
  session.sendSceneEnter(DEEP_CORRIDOR_MAP_ID, DEEP_CORRIDOR_ENTRY_X, DEEP_CORRIDOR_ENTRY_Y);
  return true;
}

function handleReceiverGhostMirrorLakeRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Receiver Ghost scene-enter map=${MIRROR_LAKE_MAP_ID} pos=${MIRROR_LAKE_RETURN_X},${MIRROR_LAKE_RETURN_Y}`
  );
  session.sendSceneEnter(MIRROR_LAKE_MAP_ID, MIRROR_LAKE_RETURN_X, MIRROR_LAKE_RETURN_Y);
  return true;
}

function handleLongicornSoldierSomberAisleRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Longicorn Soldier scene-enter map=${SOMBER_AISLE_MAP_ID} pos=${SOMBER_AISLE_ENTRY_X},${SOMBER_AISLE_ENTRY_Y}`
  );
  session.sendSceneEnter(SOMBER_AISLE_MAP_ID, SOMBER_AISLE_ENTRY_X, SOMBER_AISLE_ENTRY_Y);
  return true;
}

function handleBeetleGuideSilentHillRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Beetle Guide scene-enter map=${SILENT_HILL_MAP_ID} pos=${SILENT_HILL_ENTRY_X},${SILENT_HILL_ENTRY_Y}`
  );
  session.sendSceneEnter(SILENT_HILL_MAP_ID, SILENT_HILL_ENTRY_X, SILENT_HILL_ENTRY_Y);
  return true;
}

function handleFrontierSoldierSpringForestRequest(
  session: GameSession,
  resolvedNpcId: number,
  request: ServerRunRequestData
): boolean {
  ...
  session.log(
    `Sending Frontier Soldier scene-enter map=${SPRING_FOREST_MAP_ID} pos=${SPRING_FOREST_ENTRY_X},${SPRING_FOREST_ENTRY_Y}`
  );
